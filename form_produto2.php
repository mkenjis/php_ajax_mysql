...
<html>
  ...
  <script>
    $('#formulario_produto').submit( function(e) {
      e.preventDefault();
      var formulario = $(this);
      console.log(formulario.serialize());
      var retorno = insUpdProduto(formulario);
    });

    function insUpdProduto(dados) {
      $.ajax({
        type: "POST",
        enctype: "multipart/form-data",
        processData: false,
        data: dados.serialize(),
        url: "salvar_produto1.php",
        async: false
      }).then(sucesso,falha);

      function sucesso() {
        alert("bem sucedido");
        window.location.href = "produtos1.php"; 
      };
